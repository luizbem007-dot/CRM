...
export default function Dashboard() {
  ...
  const contacts = useMemo(
    () => [
      { id: "1", name: "Maria Silva", lastMessage: "Obrigada!", time: "09:24", unread: 2, status: "online" },
      { id: "2", name: "João Pedro", lastMessage: "Podemos reagendar?", time: "Ontem", unread: 0, status: "último visto 21:02" },
      { id: "3", name: "Clínica Viva", lastMessage: "Bot: seu agendamento foi confirmado.", time: "Seg", unread: 0, status: "bot ativo" },
    ],
    [],
  );
  ...
  const handleSend = async () => {
    ...
    messages[selectedId] = [...messages[selectedId], { id: crypto.randomUUID(), sender: "user", text: input, time: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) }];
    setInput("");
  };
    ...
    <Layout userName={userName} active={activeTab} onChange={setActiveTab}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
          ...
          <div className="lg:col-span-4 xl:col-span-3 rounded-2xl border border-border/60 bg-background/60 backdrop-blur p-3">
            <div className="flex items-center justify-between px-1 pb-2">
              ...
              <div className="text-xs text-muted-foreground">{contacts.length} contatos</div>
            </div>
          ...
          <div className="lg:col-span-8 xl:col-span-9 rounded-2xl border border-border/60 bg-background/60 backdrop-blur flex flex-col h-[70vh]">
            ...
            <div className="p-3 border-t border-border/60 flex items-center gap-2">
              <button className="h-10 w-10 rounded-xl border border-input bg-background/60 hover:bg-sidebar-accent/70"><Paperclip className="h-4 w-4 m-auto" /></button>
              <button className="h-10 w-10 rounded-xl border border-input bg-background/60 hover:bg-sidebar-accent/70" onClick={() => setInput((v) => v + " 🙂") }><Smile className="h-4 w-4 m-auto" /></button>
              <input
                className={cn("flex-1 h-11 rounded-xl bg-background/60 border border-input px-3 outline-none text-sm focus:ring-2 focus:ring-ring focus:border-transparent")}
      ...
      {activeTab === "relatorios" && (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {[
            { title: "Conversas no período", value: "1.284", sub: "+18% vs semana passada" },
            { title: "Tempo médio de resposta", value: "42s", sub: "Meta: < 60s" },
            { title: "Respostas automáticas", value: "67%", sub: "33% manuais" },
          ].map((c) => (
          ...
          <div className="xl:col-span-3 rounded-2xl border border-border/60 bg-background/60 p-5">
            <div className="text-sm text-muted-foreground">Gráfico (exemplo)</div>
      ...
      {activeTab === "configuracoes" && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <div className="rounded-2xl border border-border/60 bg-background/60 p-5 space-y-3">
            ...
            <input className="h-11 rounded-xl bg-background/60 border border-input px-3 outline-none text-sm" placeholder="FIQON_API_KEY" />
            <label className="text-sm">Webhook URL</label>
            <input className="h-11 rounded-xl bg-background/60 border border-input px-3 outline-none text-sm" placeholder="https://.../webhook" />
          ...
          <div className="rounded-2xl border border-border/60 bg-background/60 p-5 space-y-3">
            ...
            <textarea className="min-h-40 rounded-xl bg-background/60 border border-input px-3 py-2 outline-none text-sm" placeholder="Ex: Olá! Como posso ajudar?" />
            <div className="grid grid-cols-2 gap-3">
              <input className="h-11 rounded-xl bg-background/60 border border-input px-3 outline-none text-sm" placeholder="Horário início" />
              <input className="h-11 rounded-xl bg-background/60 border border-input px-3 outline-none text-sm" placeholder="Horário fim" />
            </div>
          </div>
        </div>
      )}
      ...
      {activeTab === "integracoes" && (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {[
            ...
            <div key={i.name} className="rounded-2xl border border-border/60 bg-background/60 p-5">
              <div className="flex items-center justify-between">
                <div className="font-medium">{i.name}</div>
                <div className={cn("text-xs", i.status.includes("✅") ? "text-primary" : "text-muted-foreground")}>{i.status}</div>
              </div>
